...
import static br.com.transescolar.Activies.PaisActivity.dialogPai;


public class NetworkChangeReceiver6  extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent)
    {
        try
        {
            if (isOnline(context) || isNetworkConnected(context)) {
                dialogPai(true, context);
                Log.e("Conexão", "Online Connect Intenet ");
            } else {
                dialogPai(false, context);
